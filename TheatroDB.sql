...
-- Set Row A as Wheelchair Accessible
UPDATE Seat SET isWheelchairAllow = 1 WHERE rowNo = 'A';

-- Set Rows E and above as Premium (if you have them)
-- First, find the SeatTypeId for 'Premium' in your table
DECLARE @premiumId INT = (SELECT TOP 1 seatTypeId FROM SeatType WHERE category = 'Premium');
UPDATE Seat SET seatTypeId = @premiumId WHERE rowNo >= 'E';

-- Ensure all seats are Active
UPDATE Seat SET isActive = 1;
...
-- Insert shows for Today and the next 3 days
INSERT INTO ShowTable (movieId, screenId, showDate, startTime, endTime, isCancelled)
SELECT m.movieId, sc.screenId, CAST(DATEADD(day, d.d, GETDATE()) AS DATE), t.st, t.et, 0
FROM Movie m
CROSS JOIN Screen sc
CROSS JOIN (VALUES (0),(1),(2),(3)) d(d)
CROSS JOIN (VALUES ('10:00','12:43'), ('14:00','16:43'), ('18:00','20:43'), ('21:00','23:43')) t(st, et)
WHERE NOT EXISTS (
    SELECT 1 FROM ShowTable s 
    WHERE s.movieId = m.movieId AND s.screenId = sc.screenId 
    AND s.showDate = CAST(DATEADD(day, d.d, GETDATE()) AS DATE) AND s.startTime = t.st
);

-- Sync the Seat Availability for all new shows
INSERT INTO ShowSeatAvailability (showId, seatId, isBooked)
SELECT s.showId, se.seatId, 0
FROM ShowTable s
JOIN Seat se ON se.screenId = s.screenId
WHERE NOT EXISTS (
    SELECT 1 FROM ShowSeatAvailability ssa 
    WHERE ssa.showId = s.showId AND ssa.seatId = se.seatId
);
...
INSERT INTO Feedback (userId, bookingId, rating, comments, feedbackDate)
SELECT TOP 1 u.id, b.bookingId, 5, 'Absolutely stunning visuals! A must watch.', GETDATE()
FROM Users u, Booking b WHERE u.id = b.userId;

INSERT INTO Feedback (userId, bookingId, rating, comments, feedbackDate)
SELECT TOP 1 u.id, b.bookingId, 4, 'Great story, but the theater was a bit cold.', DATEADD(day, -1, GETDATE())
FROM Users u, Booking b WHERE u.id = b.userId;
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Split Seat IDs and Insert into BookingSeat
        DECLARE @totalSeatPrice DECIMAL(10,2) = 0;
        DECLARE @SeatTable TABLE (id INT);
        INSERT INTO @SeatTable SELECT value FROM STRING_SPLIT(@seatIds, ',');

        INSERT INTO BookingSeat (bookingId, seatId, priceAtBooking)
        SELECT @bookingId, s.seatId, st.priceSeat
        FROM @SeatTable t
        JOIN Seat s ON t.id = s.seatId
        ...
        DECLARE @screenPrice DECIMAL(10,2) = (
            SELECT st.priceScreen FROM ShowTable s 
            JOIN Screen sc ON s.screenId = sc.screenId 
            JOIN ScreenType st ON sc.screenTypeId = st.screenTypeId 
            WHERE s.showId = @showId
        );

        DECLARE @finalTotal DECIMAL(10,2) = @totalSeatPrice + @screenPrice;
        
        -- Apply Tuesday Discount (if applicable)
        IF DATENAME(WEEKDAY, GETDATE()) = 'Tuesday' OR @discountCode = 'SAVE10'
            SET @finalTotal = @finalTotal * 0.9;
        ...
        -- Update Availability
        UPDATE ShowSeatAvailability SET isBooked = 1, bookedAt = GETDATE()
        WHERE showId = @showId AND seatId IN (SELECT id FROM @SeatTable);

        COMMIT;
        SELECT @bookingId AS BookingID, @finalTotal AS TotalAmount, 'Success' AS Status;
    END TRY
    BEGIN CATCH
...
CREATE OR ALTER VIEW vw_UserBookingHistory AS
SELECT 
    u.id AS UserId, u.name AS UserName, b.bookingId, b.bookingDate, b.totalAmount, b.paymentStatus,
    m.title AS MovieTitle, c.name AS CinemaName, s.showDate, s.startTime,
...
-- 2. Insert Hardcoded Starter Reviews
INSERT INTO Feedback (userId, bookingId, rating, comments, feedbackDate)
SELECT TOP 2 u.id, b.bookingId, 5, 'Absolutely loved the IMAX experience! Sound quality was incredible.', GETDATE()
FROM Users u CROSS JOIN Booking b WHERE u.id = b.userId;

INSERT INTO Feedback (userId, bookingId, rating, comments, feedbackDate)
SELECT TOP 1 u.id, b.bookingId, 4, 'Very comfortable seats and the 3D effects were great.', DATEADD(day, -1, GETDATE())
FROM Users u CROSS JOIN Booking b WHERE u.id = b.userId;
/* ===============================================================
   THEATRO CINEMA SYSTEM - BCNF COMPLIANT SCHEMA
   =============================================================== */
...
-- 3. FIX: Ensure My Bookings shows everything correctly (Update the View)
CREATE OR ALTER VIEW vw_UserBookingHistory AS
SELECT 
    u.id AS UserId, 
    u.name AS UserName, 
...
-- Insert hardcoded reviews/feedback
INSERT INTO Feedback (userId, bookingId, rating, comments, feedbackDate) VALUES
(1, 1, 5, 'Absolutely amazing movie! The IMAX experience was incredible. Will definitely come back.', GETDATE()),
(2, 2, 4, 'Great film but the seats were a bit uncomfortable. Overall good experience.', GETDATE()),
(1, 3, 5, 'Best cinema in town! Sound quality was perfect and staff was very helpful.', DATEADD(day, -1, GETDATE())),
(3, 1, 3, 'Movie was good but the popcorn was cold. Please improve food service.', DATEADD(day, -2, GETDATE())),
(2, 4, 4, 'Loved the 3D effects! Screen quality is excellent. Will recommend to friends.', DATEADD(day, -3, GETDATE())),
(4, 2, 5, 'Very clean theater and comfortable seating. Great experience overall.', DATEADD(day, -4, GETDATE())),
(3, 2, 2, 'Movie started late and the AC was too cold. Disappointed with management.', DATEADD(day, -5, GETDATE())),
(1, 2, 5, 'Perfect date night spot! The VIP seating is worth every penny.', DATEADD(day, -6, GETDATE())),
(4, 3, 4, 'Good value for money. Online booking system works smoothly.', DATEADD(day, -7, GETDATE())),
(2, 2, 5, 'Watched Oppenheimer here. The sound system made the experience unforgettable!', DATEADD(day, -8, GETDATE()));
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 1. Identify the Show and Seats
        DECLARE @showId INT = (SELECT showId FROM Booking WHERE bookingId = @bookingId AND userId = @userId);
        
        IF @showId IS NULL
            THROW 50008, 'Booking ID not found in database for this user.', 1;

        -- 2. Release the Seats (Make them available again)
        UPDATE ShowSeatAvailability 
        SET isBooked = 0, bookedAt = NULL
        WHERE showId = @showId 
        AND seatId IN (SELECT seatId FROM BookingSeat WHERE bookingId = @bookingId);
