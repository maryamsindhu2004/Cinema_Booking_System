...
async function initDb() {
    try {
        ...
        // Check if we already have data to avoid wiping it
        const checkMovies = await pool.request().query("SELECT COUNT(*) as count FROM Movie");
        if (checkMovies.recordset[0].count > 0) {
        ...
        // 15. ShowSeatAvailability table (tracks live seat status per show)
        await pool.request().query(`
            IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='ShowSeatAvailability' AND xtype='U')
            BEGIN
