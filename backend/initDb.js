...
async function initDb() {
    try {
        ...
        const tables = [
            ...
            { name: 'Feedback', query: `CREATE TABLE Feedback (feedbackId INT PRIMARY KEY IDENTITY(1,1), userId INT NOT NULL, rating INT CHECK (rating >= 1 AND rating <= 5), comments NVARCHAR(MAX), submitted_at DATETIME DEFAULT GETDATE(), FOREIGN KEY (userId) REFERENCES Users(id) ON DELETE CASCADE)` }
        ];
